...
from eval_spot import FasterTT_Planner
###############
...
ACTION_LOG_PATH='logs-100-img4/'+dt_string+'/'
STATE_LOG_PATH='logs-100-img4/'+dt_string+'/'
ARM_LOG_PATH='logs-100-img4/'+dt_string+'/arms/' 
IMG_LOG_PATH='logs-100-img4/'+dt_string+'/image/'
...
class WasdInterface(object):
    ...
    def __init__(self, robot):
        ...
        self._command_dictionary = {
            ...
            #ord('W'): self._arm_move_out,
            #ord('S'): self._arm_move_in,
            ...
            #ord('1'): self._body_yaw_left,
            #ord('1'): self._create_folder,
                ...
                while not self._exit_check.kill_now:
                    ...
                    if not AI_CONTROL:
                        ...
                        try:
                            ...
                            cam_names = ['frontright_fisheye_image', 'frontleft_fisheye_image',
                                         'frontright_depth_in_visual_frame', 'frontleft_depth_in_visual_frame']
                            im_res = self.ai_image_client.get_image_from_sources(cam_names)
                            image_r = Image.open(io.BytesIO(im_res[0].shot.image.data)).convert('RGB')
                            image_l = Image.open(io.BytesIO(im_res[1].shot.image.data)).convert('RGB')
                            image_dr = np.frombuffer(im_res[2].shot.image.data, dtype=np.uint16).reshape(
                                im_res[2].shot.image.rows, im_res[2].shot.image.cols)
                            image_dl = np.frombuffer(im_res[3].shot.image.data, dtype=np.uint16).reshape(
                                im_res[2].shot.image.rows, im_res[2].shot.image.cols)

                            obs = self.encoder.encode_image_depth_state(img_l=image_l, img_r=image_r, dep_r=image_dr,
                                                                        dep_l=image_dl, state=arm)

                            #self.planner.plan(obs,time_step)
                            action = self.planner.get_action(obs,time_step)
                            time_step = time_step+1
                            cmd = num_to_com[int(action)]
                            print(cmd)
                            self.flush_and_estop_buffer(stdscr)
                            self._drive_cmd(ord(cmd))
                            time.sleep(COMMAND_INPUT_RATE)
